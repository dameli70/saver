...
<title>LOCKSMITH — Zero-Knowledge Vault</title>
...
<!-- ══ AUTH ══ -->
<div id="auth-screen" <?= $loggedIn ? 'style="display:none"' : '' ?>>
  <div class="auth-box">
    <div class="auth-logo">LOCK<span>SMITH</span></div>
    <div class="auth-sub">// Zero-Knowledge Vault</div>
    <div class="zk-badge">🔐 Your key never leaves your device</div>
    ...
    <div id="pane-login">
      <div class="field"><label>Email</label>
        <input type="email" id="l-email" placeholder="you@example.com" autocomplete="email" inputmode="email">
      </div>
      <div class="field"><label>Login Password</label>
        <input type="password" id="l-pwd" placeholder="••••••••" autocomplete="current-password">
      </div>
      <button class="btn btn-primary" onclick="doLogin()"><span id="l-txt">Login</span></button>
    </div>

    <!-- REGISTER -->
    <div id="pane-reg" style="display:none">
      <div class="sec-callout">
        You need <strong>two separate credentials</strong>:<br>
        • <strong>Login Password</strong> — to authenticate to this site.<br>
        • <strong>Vault Passphrase</strong> — used only in your browser to encrypt/decrypt passwords. <strong>It is never sent to the server.</strong> If you forget it, your passwords cannot be recovered by anyone.
      </div>
      <div class="field"><label>Email</label>
      ...
      <div class="field"><label>Vault Passphrase <span style="color:var(--muted)">(min 10 chars, never transmitted)</span></label>
        <input type="password" id="r-vault" placeholder="Something memorable only you know" autocomplete="new-password">
        <div class="field-note">⚠ Write this down somewhere physical. Loss = permanent loss of all passwords.</div>
      </div>
      <button class="btn btn-primary" onclick="doRegister()"><span id="r-txt">Create Account</span></button>
    </div>
  </div>
</div>

<!-- ══ APP ══ -->
<div id="app" <?= $loggedIn ? 'class="show"' : '' ?>>
  ...
  <div class="app-body">
    ...
    <div class="card" id="vault-unlock-card" style="display:none">
      ...
      <p style="font-size:12px;color:var(--muted);margin-bottom:14px;line-height:1.6;">
        Your vault passphrase is required to generate or reveal passwords. It is used only in your browser — never sent to the server.
      </p>
    ...
    <div class="card card-accent" id="gen-card">
      <div class="card-title"><div class="dot"></div>Generate New Lock</div>
      ...
      <div class="field"><label>Memory Hint <span style="color:var(--muted);font-size:10px;">(optional — never the password)</span></label>
    ...
    <div class="sec-header">
      <div class="card-title" style="margin-bottom:0"><div class="dot"></div>My Locks</div>
      <button class="btn btn-ghost btn-sm" onclick="loadLocks()">↻</button>
    </div>
    <div id="locks-wrap">
      <div class="empty"><div class="empty-icon">🔒</div><h3>No locks yet</h3><p>Generate your first lock above.</p></div>
    </div>
...
<div id="confirm-overlay">
  <div class="confirm-sheet">
    <div class="confirm-title">Did you paste the password?</div>
    <div class="confirm-sub" id="cs-sub">Encrypted password was copied to your clipboard.</div>
    <div class="timer-wrap">
      ...
      <div class="timer-msg" id="timer-msg">Confirm within <strong>2 minutes</strong> that you've pasted this into your app.</div>
    </div>
    <div class="autosave-bar" id="autosave-bar">⏰ Auto-saved. Password stored but <strong>not time-locked</strong> until you confirm below.</div>
    <div class="void-box" id="void-box">
      <div class="void-label">// Void Password — browser-decrypted for reference only</div>
      <div class="void-pwd" id="void-pwd"></div>
      <div class="void-note">This password was never confirmed — it is not enforced or locked.</div>
    </div>
    <div class="confirm-btns" id="confirm-btns">
      <button class="btn btn-green" onclick="doConfirm('confirm')">✓ Yes, I pasted it</button>
...
<div id="reveal-overlay" onclick="closeReveal(event)">
  <div class="reveal-sheet">
    ...
    <button class="btn btn-full" id="rv-copy-btn" style="margin-top:10px;background:var(--s3);border:1px solid var(--b2);color:var(--text);display:none;" onclick="copyRevealed()">Copy Password</button>
    <div id="rv-zk-note" style="font-size:10px;color:var(--muted);margin-top:12px;letter-spacing:.5px;line-height:1.5;display:none;">
      🔐 Decrypted entirely in your browser. This server never saw this password.
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('r-vault').addEventListener('keydown', e => { if(e.key==='Enter') doRegister(); });
...
function authTab(t) {
  ...
  document.getElementById('pane-reg').style.display   = t==='register' ? 'block':'none';
  document.querySelectorAll('.auth-tab').forEach((el,i)=>
    el.classList.toggle('active',(i===0&&t==='login')||(i===1&&t==='register')));
  clearAuthMsgs();
}
function clearAuthMsgs(){['auth-err','auth-ok'].forEach(id=>document.getElementById(id).classList.remove('show'));}
function authErr(m){const el=document.getElementById('auth-err');el.textContent=m;el.classList.add('show');}

async function doLogin(){
  const email=document.getElementById('l-email').value.trim();
  const pwd=document.getElementById('l-pwd').value;
  clearAuthMsgs();
  if(!email||!pwd){authErr('Fill in all fields');return;}
  document.getElementById('l-txt').innerHTML='<span class="spin light"></span>';
  try{
    const r=await post('/api/auth.php',{action:'login',login_password:pwd,email});
    if(r.success){document.getElementById('u-email').textContent=r.email;showApp();loadLocks();checkVaultUnlock();}
    else authErr(r.error||'Login failed');
  }catch{authErr('Network error');}
  finally{document.getElementById('l-txt').textContent='Login';}
}

async function doRegister(){
  const email=document.getElementById('r-email').value.trim();
  const pwd=document.getElementById('r-pwd').value;
  const vault=document.getElementById('r-vault').value;
  clearAuthMsgs();
  if(!email||!pwd||!vault){authErr('Fill in all fields');return;}
  if(pwd.length<8){authErr('Login password: min 8 characters');return;}
  if(vault.length<10){authErr('Vault passphrase: min 10 characters');return;}
  if(pwd===vault){authErr('Vault passphrase must differ from login password');return;}

  document.getElementById('r-txt').innerHTML='<span class="spin light"></span>';
  try{
    const r=await post('/api/auth.php',{action:'register',email,login_password:pwd,vault_passphrase:vault});
    if(r.success){document.getElementById('u-email').textContent=r.email;showApp();loadLocks();checkVaultUnlock();}
    else authErr(r.error||'Registration failed');
  }catch{authErr('Network error');}
  finally{document.getElementById('r-txt').textContent='Create Account';}
}
...
//  3. Generate random password IN BROWSER
//  4. Encrypt password IN BROWSER (AES-256-GCM)
//  5. Send ONLY ciphertext to server — no key, no plaintext
//  6. Copy ciphertext-decryptable output to clipboard
// ─────────────────────────────────────────────────
...
async function doGenerate(){
  ...
  try {
    ...
    document.getElementById('kdf-label').textContent='Generating password…';
    ...
    // Step 7: Copy plaintext password to clipboard (blind — not shown in UI)
    try {
      await navigator.clipboard.writeText(plainPwd);
      toast('Password copied to clipboard (never displayed)', 'ok');
    } catch {
      // Fallback
      ...
      toast('Password copied (tap to verify paste worked)','warn');
    }
...
async function triggerAutoSave(lockId){
  ...
  document.getElementById('timer-msg').innerHTML='⏰ Auto-saved. <strong>Not time-locked</strong> until you confirm.';
  loadLocks();
}
...
async function doConfirm(action){
  ...
  if(action==='confirm'){
    ...
    msg.innerHTML=`✓ Lock activated.<br>Password sealed until<br><strong style="color:var(--accent)">${ds}</strong>`;
    loadLocks();
  } else if(action==='reject'){
    msg.innerHTML=`✗ Discarded. Decrypting void password in your browser…`;
    // Decrypt void password in browser to show user (passphrase + blobs from pendingLock)
    if(pendingLock.cipher_blob && vaultPhraseSession){
      try{
        ...
        msg.innerHTML=`✗ Password discarded. The void password is shown below — it was <strong>never locked</strong>.`;
      }catch{
        msg.innerHTML=`✗ Discarded. Could not decrypt void password.`;
      }
    }
    loadLocks();
  }
...
async function loadLocks(){
  ...
  try{
    ...
    if(!r.success||!r.locks.length){
      wrap.innerHTML='<div class="empty"><div class="empty-icon">🔒</div><h3>No locks yet</h3><p>Generate your first lock above.</p></div>';
      return;
    }
  ...
  }catch{wrap.innerHTML='<div class="empty"><p>Failed to load.</p></div>';}
}
